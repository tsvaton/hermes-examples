...
int main(int argc, char* argv[])
{
  ...
  if (bt.is_embedded() == false && ADAPTIVE_TIME_STEP_ON == true) {
    Hermes::Mixins::Loggable::static_warn("R-K method not embedded, turning off adaptive time stepping.");
  ...
  if (ADAPTIVE_TIME_STEP_ON) Hermes::Mixins::Loggable::static_info("Time step history will be saved to file time_step_history.dat.");
  ...
  do 
  {
    Hermes::Mixins::Loggable::static_info("Begin time step %d.", ts);
    ...
    do {
      ...
      Hermes::Mixins::Loggable::static_info("Runge-Kutta time step on fine mesh (t = %g s, time step = %g s, stages: %d).", 
      ...
      if (bt.is_embedded() == true) {
        Hermes::Mixins::Loggable::static_info("Calculating temporal error estimate.");
        ...
        if (ADAPTIVE_TIME_STEP_ON == false) Hermes::Mixins::Loggable::static_info("rel_err_time: %g%%", rel_err_time);
      }

      if (ADAPTIVE_TIME_STEP_ON) {
        if (rel_err_time > TIME_ERR_TOL_UPPER) {
          Hermes::Mixins::Loggable::static_info("rel_err_time %g%% is above upper limit %g%%", rel_err_time, TIME_ERR_TOL_UPPER);
          Hermes::Mixins::Loggable::static_info("Decreasing time step from %g to %g s and restarting time step.", 
          ...
          Hermes::Mixins::Loggable::static_info("rel_err_time = %g%% is below lower limit %g%%", rel_err_time, TIME_ERR_TOL_LOWER);
          Hermes::Mixins::Loggable::static_info("Increasing time step from %g to %g s.", time_step, time_step * TIME_STEP_INC_RATIO);
          ...
          Hermes::Mixins::Loggable::static_info("rel_err_time = %g%% is in acceptable interval (%g%%, %g%%)", 
      ...
      Hermes::Mixins::Loggable::static_info("Spatial adaptivity step %d.", as);

      // Project the fine mesh solution onto the coarse mesh.
      Solution<std::complex<double> > sln;
      Hermes::Mixins::Loggable::static_info("Projecting fine mesh solution on coarse mesh for error estimation.");
      ...
      Hermes::Mixins::Loggable::static_info("Calculating spatial error estimate.");
      Adapt<std::complex<double> >* adaptivity = new Adapt<std::complex<double> >(&space);
      double err_rel_space = adaptivity->calc_err_est(&sln, &ref_sln) * 100;

      // Report results.
      Hermes::Mixins::Loggable::static_info("ndof: %d, ref_ndof: %d, err_rel_space: %g%%", 
