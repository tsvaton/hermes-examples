  ...
  for(t = 0.0; t < 10.0; t += time_step_after_adaptivity)
  {
    ...
    do
    {
      ...
      if(SEMI_IMPLICIT)
      {
        ...
        if(SHOCK_CAPTURING && SHOCK_CAPTURING_TYPE == FEISTAUER)
        {
          Hermes::Mixins::Loggable::static_info("Assembling the stabilization right-hand side vector.");
      ...
      Hermes::Mixins::Loggable::static_info("Calculating error estimates.");
      ...
      // Report results.
      Hermes::Mixins::Loggable::static_info("Error estimate for the flow part: %g%%", err_est_rel_total_flow);

      Hermes::Mixins::Loggable::static_info("Error estimate for the concentration part: %g%%", err_est_rel_total_concentration);
      ...
      {
        Hermes::Mixins::Loggable::static_info("Adapting coarse meshes.");
          ...
          &space_rho_v_y, &space_e, &space_c)) >= NDOF_STOP)
        {
          ...
          Hermes::Mixins::Loggable::static_info("Maximum number of dofs of the coarse meshes, %i, has been reached, adaptivity loop ends.", Space<double>::get_num_dofs(Hermes::vector<const Space<double> *>(&space_rho, &space_rho_v_x, 
