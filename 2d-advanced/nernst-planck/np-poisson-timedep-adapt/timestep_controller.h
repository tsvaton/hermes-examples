...
void PidTimestepController::begin_step() {
  ...
  if ((time + (*timestep)) >= final_time) {
   Hermes::Mixins::Loggable::static_info("Time step would exceed the final time... reducing");
    (*timestep) = final_time - time;
   Hermes::Mixins::Loggable::static_info("The last time step: %g", *timestep);
    finished = true;
  }
  time += (*timestep);
  step_number++;
 Hermes::Mixins::Loggable::static_info("begin_step processed, new step number: %i and cumulative time: %g", step_number, time);
}
...
bool PidTimestepController::end_step(Hermes::vector<Solution<double> *> solutions,
  ...
  if (pid) {
    ...
    for (unsigned int i = 0; i < neq; i++) {
     ...
     Hermes::Mixins::Loggable::static_info("Solution[%i]: rel error %g, largest relative error %g",
    ...
    if (err_vector.size() > 2 && max_rel_error <= delta) {
     ...
     Hermes::Mixins::Loggable::static_info("Error vector sufficient for adapting...");
      ...
      Hermes::Mixins::Loggable::static_info("Coefficient %g", t_coeff);
       (*timestep) = (*timestep)*t_coeff;
      Hermes::Mixins::Loggable::static_info("New time step: %g", *timestep);
    }
  } // end pid
