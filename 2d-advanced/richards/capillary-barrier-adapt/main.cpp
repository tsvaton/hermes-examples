...
int main(int argc, char* argv[])
{
  ...
  if (ITERATIVE_METHOD == 1) {
    if (TIME_INTEGRATION == 1) {
      Hermes::Mixins::Loggable::static_info("Creating weak formulation for the Newton's method (implicit Euler in time).");
      wf = new WeakFormRichardsNewtonEuler(&constitutive_relations, time_step, &sln_prev_time, &mesh);
    }
    else {
      Hermes::Mixins::Loggable::static_info("Creating weak formulation for the Newton's method (Crank-Nicolson in time).");
      wf = new WeakFormRichardsNewtonCrankNicolson(&constitutive_relations, time_step, &sln_prev_time, &mesh);
    }
  }
  else {
    if (TIME_INTEGRATION == 1) {
      Hermes::Mixins::Loggable::static_info("Creating weak formulation for the Picard's method (implicit Euler in time).");
      wf = new WeakFormRichardsPicardEuler(&constitutive_relations, time_step, &sln_prev_iter, &sln_prev_time, &mesh);
    }
    else {
      Hermes::Mixins::Loggable::static_info("Creating weak formulation for the Picard's method (Crank-Nicolson in time).");
  ...
  {
    Hermes::Mixins::Loggable::static_info("---- Time step %d:", ts);
    ...
    do
    {
      Hermes::Mixins::Loggable::static_info("---- Time step %d, time step lenght %g, time %g (days), adaptivity step %d:", ts, time_step, current_time, as);
      ...
      if(ITERATIVE_METHOD == 1) {
        ...
        if (as == 1 && ts == 1) {
          Hermes::Mixins::Loggable::static_info("Projecting coarse mesh solution to obtain initial vector on new fine mesh.");
          OGProjection<double> ogProjection; ogProjection.project_global(ref_space, &sln_prev_time, coeff_vec);
        }
        else {
          Hermes::Mixins::Loggable::static_info("Projecting previous fine mesh solution to obtain initial vector on new fine mesh.");
        ...
        Hermes::Mixins::Loggable::static_info("Performing Newton's iteration (tau = %g days):", time_step);
        ...
        while(!newton_converged)
        {
          ...
          if(!newton_converged)
          {
            ...
            Hermes::Mixins::Loggable::static_info("Reducing time step size from %g to %g days for the rest of this time step.", 
        ...
        // reduce time step to make it converge, but then restore time step 
        // size to its original value.
        Hermes::Mixins::Loggable::static_info("Performing Picard's iteration (tau = %g days):", time_step);
        bool verbose = true;
        ...
        while(true)
        {
          ...
          catch(std::exception& e)
          {
            ...
            // Reducing time step to 50%.
            Hermes::Mixins::Loggable::static_info("Reducing time step size from %g to %g days for the rest of this time step", time_step, time_step * time_step_inc);
            time_step *= time_step_dec;
      ...
      Hermes::Mixins::Loggable::static_info("Projecting fine mesh solution on coarse mesh for error calculation.");
      ...
      // Report results.
      Hermes::Mixins::Loggable::static_info("ndof_coarse: %d, ndof_fine: %d, space_err_est_rel: %g%%", 
    ...
    if (time_step*time_step_inc < time_step_max) {
      Hermes::Mixins::Loggable::static_info("Increasing time step from %g to %g days.", time_step, time_step * time_step_inc);
